...
async function resolveProject(api: BasecampAPI, index: IndexManager, ref: string): Promise<ProjectIndex | null> {
  ...
  if (/^\d+$/.test(ref)) {
    ...
    while (page <= 10) {
      ...
      if (!Array.isArray(pageProjects) || pageProjects.length === 0) break;
      const match = pageProjects.find((p: any) => p.name.toLowerCase().includes(lowerQuery));
      if (match) { projectId = match.id.toString(); break; }
...
async function getAssignedCards(projectQuery: string, userName: string) {
  ...
  while (page <= 10) {
    const response = await api.getProjects(undefined, page);
    const pageProjects = response.data;
    if (!pageProjects || !Array.isArray(pageProjects) || pageProjects.length === 0) break;
    totalChecked += pageProjects.length;
    const matches = pageProjects.filter((p: any) => p.name.toLowerCase().includes(lowerQuery));
    if (matches.length > 0) {
...
async function findProject(query: string) {
  ...
  while (page <= 10) {
    ...
    const pageProjects = response.data;
    if (!Array.isArray(pageProjects) || pageProjects.length === 0) break;
    matches.push(...pageProjects.filter((p: any) => p.name.toLowerCase().includes(lower)));
    if (pageProjects.length < 100) break;
    page++;
  }
